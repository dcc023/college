#UPDATED COMPILER BY DYLAN CAMPBELL
...
def error(num):
    ...
    if num == 1: 
    	...
    	print >>outfile, "Left parenthesis expected."
    elif num == 29:
    	print >>outfile, "Missing to or downto."
    elif num == 30:
    	print >>outfile, "Expected of."
    elif num == 31:
    	print >>outfile, "Ident needs to be a constant."
    elif num == 32:
...
def block(tableIndex):
    ...
    tx[0] = tableIndex
    global sym, id;
    while sym == "CONST" or sym == "VAR" or sym == "PROC": #restricted globals
	    if sym == "CONST":
...
def statement(tx):
    ...
    if sym == "ident":
    ...
    elif sym == "REPEAT": #REPEAT-UNTIL
    	while True: #So we can chain statements after semicolons
    		getsym()
    		statement(tx)
    		if sym != "semicolon": #breaks free without semicolon, moves on
    			break
    	if sym != "UNTIL": #error checks for UNTIL
    		error(27)
    	getsym()
    	condition(tx)

    elif sym == "WRITE" or sym == "WRITELN": # WRITE or WRITELN, since they are read the same
    	getsym()
    	if sym != "lparen": # error check for left parenthesis
    		error(28)
    	while True: # Chaining expressions after commas
    		getsym()
    		expression(tx)
    		if sym != "comma": #breaks free without comma, moves on
    			break
    	if sym != "rparen":
    		error(22)
    	getsym()
    	...
    	if i==0:
        	error(11)
    	if table[i].kind != "variable": #checks for ident to be a variable
        	error(12)
    	getsym()
    	if sym != "becomes":
        	error(13)
    	...
    	while sym == "number" or sym == "ident":
    		if sym == "ident":
	    		...
	    		if i==0:
	        		error(11)
	    		elif table[i].kind != "const": #checks for ident to be a constant
	        		error(31)
	    	getsym()
    		if sym != "colon":
    			error(32)
